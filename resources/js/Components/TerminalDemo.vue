<template>
  <section class="py-24 border-b border-outline-variant/15 bg-surface relative overflow-hidden">
    <!-- Noise overlay -->
    ...
    <div class="max-w-7xl mx-auto px-6 relative z-10">
      <div class="mb-16 text-center">
        <h2 class="text-4xl md:text-5xl font-black uppercase tracking-tighter text-on-surface drop-shadow-lg">Watch It Happen. Live.</h2>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch relative">
        ...
        <div class="hidden lg:block absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-px bg-primary z-0 shadow-[0_0_10px] shadow-pulse"></div>

        <!-- Terminal Side -->
        <div class="bg-surface-container-lowest border border-outline-variant/15 p-1 shadow-2xl flex flex-col min-h-[450px] relative group">
          <div class="absolute inset-0 border border-primary/30 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"></div>
          
          <div class="bg-surface-container-low p-2 flex items-center justify-between border-b border-outline-variant/15">
            <div class="flex items-center gap-2">
              <div class="w-3 h-3 bg-surface-container-lowest-border rounded-full flex items-center justify-center"><div class="w-1 h-1 bg-surface rounded-full"></div></div>
              <div class="w-3 h-3 bg-surface-container-lowest-border rounded-full flex items-center justify-center"><div class="w-1 h-1 bg-surface rounded-full"></div></div>
              <div class="w-3 h-3 bg-surface-container-lowest-border rounded-full flex items-center justify-center"><div class="w-1 h-1 bg-surface rounded-full"></div></div>
            </div>
            <span class="font-mono text-[10px] text-on-surface-variant tracking-widest uppercase">root@noryxon-node:~</span>
          </div>
          
          <div class="font-mono text-sm text-on-surface flex-1 overflow-x-auto p-6 relative">
            ...
            <div class="relative z-10">
              <p class="mb-4 text-on-surface-variant"># CREATE_INVOICE_FOR_TRANSACTION</p>
              <p><span class="text-primary-container font-bold">curl</span> -X POST https://api.noryxon.com/v1/invoices \</p>
              <p class="ml-4">-H <span class="text-primary/90">"Authorization: Bearer sk_live_7x9a..."</span> \</p>
              <p class="ml-4">-H <span class="text-primary/90">"Content-Type: application/json"</span> \</p>
              <p class="ml-4">-d '{</p>
              <p class="ml-8 text-on-surface/90">"amount": "{{ currentAmount }}",</p>
              <p class="ml-8 text-on-surface/90">"currency": "{{ currentCurrency }}",</p>
              <p class="ml-8 text-on-surface/90">"chain": "{{ currentChain }}",</p>
              <p class="ml-8 text-on-surface/90">"payer": "client@example.com"</p>
              <p class="ml-4">}'</p>
              
              <div class="mt-8 border-t border-dashed border-outline-variant/15 pt-4">
                <div v-if="!isSimulating && !isComplete" class="text-primary animate-pulse">
                  <span class="mr-2">_</span>READY_TO_GENERATE_INVOICE
                </div>
                
                <div v-if="isSimulating" class="text-primary-container">
                  <p v-for="(log, idx) in simLogs" :key="idx" class="mb-1 opacity-80">> {{ log }}</p>
                  <p class="animate-pulse">> _</p>
                </div>
                
                <div v-if="isComplete" class="text-primary text-xs leading-relaxed">
                   > HTTP/1.1 200 OK<br>
                   > <span class="text-on-surface">{</span><br>
        ...
        <div class="bg-surface border border-outline-variant/15 p-1 flex flex-col justify-center items-center relative min-h-[450px] shadow-2xl group">
          <!-- Scanning beam -->
          ...
          <div class="absolute top-4 right-4 flex items-center gap-2">
            <span class="w-2 h-2 rounded-full" :class="isComplete ? 'bg-primary shadow-[0_0_8px] shadow-pulse' : 'bg-node shadow-[0_0_8px] shadow-node animate-pulse'"></span>
            <span class="font-mono text-[10px] tracking-widest text-on-surface-variant">SESSION_ACTIVE</span>
          </div>
          ...
          <div class="text-center mt-8 z-10 w-full px-8">
            ...
            <p class="font-mono text-on-surface-variant mb-8 text-xs border-b border-outline-variant/15 pb-4 w-full">TRANSFER_EXPECTED: {{ currentAmount }} {{ currentCurrency }} ({{ currentChain.toUpperCase() }})</p>
            
            <div class="relative w-48 h-48 mx-auto mb-8 bg-surface border border-outline-variant/15 flex items-center justify-center p-2 group-hover:border-node/50 transition-colors">
              ...
              <div v-if="isComplete" class="absolute inset-0 flex items-center justify-center">
                <div class="w-16 h-16 bg-primary text-void rounded-full flex items-center justify-center animate-[popIn_0.5s_cubic-bezier(0.175,0.885,0.32,1.275)] shadow-[0_0_30px] shadow-pulse">
            ...
            <div class="font-mono bg-surface-container-low border border-outline-variant/15 px-4 py-3 w-full max-w-sm mx-auto truncate text-on-surface/90 text-xs tracking-wider relative overflow-hidden group/addr">
              <div class="absolute inset-0 bg-primary/10 translate-y-full group-hover/addr:translate-y-0 transition-transform"></div>
              <span class="relative z-10">{{ isComplete ? 'INVOICE_DOCUMENTED' : currentAddress }}</span>
            </div>
            
            <div class="mt-8 mx-auto font-mono text-[10px] uppercase font-bold px-6 py-3 tracking-widest text-center border" :class="isComplete ? 'border-node text-primary-container shadow-[0_0_15px] shadow-node/30' : 'border-primary/50 text-primary/80'">
