<template>
  <DashboardLayout pageTitle="Wallets" breadcrumb="SYSTEM > WALLETS > VAULT" dashboardType="merchant">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <div class="text-sm font-medium text-on-surface-variant mb-1">Link wallets and exchanges for automatic transaction detection and invoice generation. We never touch your funds — pinky promise.</div>
      </div>
      <button
    ...
    <!-- Filter Tabs -->
    <div class="flex items-center gap-1 mb-6 p-1 bg-surface-container-lowest rounded-lg border border-outline-variant/15 w-fit">
      <button
        ...
        :class="activeFilter === tab.value
          ? 'bg-surface text-on-surface shadow-sm border border-outline-variant/15'
    ...
    <div v-if="filteredWallets.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div
        ...
        :key="wallet.id"
        class="border border-outline-variant/15 bg-surface rounded-xl p-5 hover:border-primary/30 transition-all group/card"
      >
        <!-- Card Header -->
        <div class="flex items-start justify-between mb-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <div class="text-sm font-bold text-on-surface">{{ wallet.label }}</div>
              <div class="flex items-center gap-2 mt-0.5">
                ...
                <span v-if="wallet.chain" class="text-[10px] text-on-surface-variant font-medium">
                  {{ getChainConfig(wallet.chain).icon }} {{ getChainConfig(wallet.chain).symbol }}
                </span>
                <span v-else class="text-[10px] text-on-surface-variant font-medium">Multi-chain</span>
              </div>
            </div>
          </div>
          <!-- Status Dot -->
        ...
        <!-- Identifier -->
        <div class="mb-4 p-2.5 bg-surface-container-lowest rounded-lg border border-outline-variant/15">
          <div class="text-[10px] text-on-surface-variant font-semibold mb-1 uppercase tracking-wide">
            {{ wallet.type === 'exchange' ? 'API Connection' : wallet.type === 'xpub' ? 'Extended Public Key' : 'Address' }}
          </div>
          <div class="text-xs font-mono text-primary truncate cursor-pointer hover:underline" @click="copyIdentifier(wallet)">
            {{ wallet.displayIdentifier }}
          </div>
          <div v-if="wallet.derivationPath" class="text-[10px] font-mono text-on-surface-variant mt-1">
        ...
        <div class="grid grid-cols-2 gap-3 mb-4">
          <div>
            <div class="text-[10px] text-on-surface-variant font-medium uppercase tracking-wide">Received</div>
            <div class="text-sm font-bold text-on-surface">{{ formatCurrency(wallet.totalReceived) }}</div>
          </div>
          <div>
            <div class="text-[10px] text-on-surface-variant font-medium uppercase tracking-wide">
        ...
        <!-- Actions -->
        <div class="flex items-center gap-2 pt-3 border-t border-outline-variant/15">
          <button
            @click="toggleStatus(wallet)"
            class="flex-1 px-3 py-2 rounded-lg border border-outline-variant/15 text-on-surface-variant hover:border-blue-500 hover:text-blue-500 transition-colors text-xs font-semibold"
          >
            {{ wallet.status === 'paused' ? '▶ Resume' : '⏸ Pause' }}
          </button>
          <button
            @click="deleteWallet(wallet)"
            class="px-3 py-2 rounded-lg border border-outline-variant/15 text-on-surface-variant hover:border-red-500 hover:text-red-500 transition-colors text-xs font-semibold"
          >
            ✕
          </button>
        </div>
    ...
    <!-- Empty State -->
    <div v-else class="border border-outline-variant/15 border-dashed bg-surface rounded-xl p-12 text-center">
      ...
      <button
        @click="openConnectModal"
        class="inline-flex items-center gap-2 bg-primary text-void font-semibold px-5 py-2.5 rounded-lg text-sm hover:shadow-lg hover:shadow-pulse/20 transition-all"
      >
        Connect Wallet
      </button>
    </div>
    ...
    <Teleport to="body">
      ...
      >
        <div v-if="showConnectModal" class="fixed inset-0 z-50 flex items-center justify-center bg-surface/80 backdrop-blur-sm" @click.self="closeModal">
          <div class="bg-surface-container-lowest border border-outline-variant/15 rounded-xl w-full max-w-2xl mx-4 shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
            <!-- Modal Header -->
            <div class="flex items-center justify-between px-6 py-4 border-b border-outline-variant/15 bg-surface sticky top-0 z-10">
              ...
              <div class="flex items-center gap-2">
                <button v-if="modalStep === 2" @click="modalStep = 1" class="px-3 py-1 rounded-md text-xs font-medium text-on-surface-variant hover:text-on-surface hover:bg-surface-container-low transition-colors">
                  ← Back
                </button>
                <button @click="closeModal" class="p-1 rounded-md text-on-surface-variant hover:text-on-surface hover:bg-surface-container-low transition-colors">
            ...
            <div v-if="modalStep === 1" class="p-6">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <button
                  ...
                  class="p-5 border border-outline-variant/15 rounded-xl text-left hover:border-primary hover:bg-surface transition-all group/opt"
                >
            ...
            <div v-if="modalStep === 2" class="p-6 space-y-5 bg-surface/50">

              <!-- EXCHANGE FORM -->
              <template v-if="form.type === 'exchange'">
                <div>
                  <label class="block text-xs font-semibold text-on-surface mb-2">Exchange</label>
                ...
                <div>
                  ...
                  <input v-model="form.label" type="text" placeholder="e.g. My Binance Account" class="w-full bg-surface border border-outline-variant/15 rounded-lg px-4 py-2.5 text-sm text-on-surface placeholder:text-on-surface-variant/50 focus:border-primary focus:ring-1 focus:ring-pulse focus:outline-none transition-all" />
                  <div v-if="form.errors.label" class="text-red-500 text-xs mt-1">{{ form.errors.label }}</div>
                </div>
                <div>
                  <label class="block text-xs font-semibold text-on-surface mb-2">API Key <span class="text-on-surface-variant font-normal">(read-only recommended)</span></label>
                  <input v-model="form.api_key" type="password" placeholder="Enter your API key" class="w-full bg-surface border border-outline-variant/15 rounded-lg px-4 py-2.5 text-sm font-mono text-on-surface placeholder:text-on-surface-variant/50 focus:border-primary focus:ring-1 focus:ring-pulse focus:outline-none transition-all" />
                  <div v-if="form.errors.api_key" class="text-red-500 text-xs mt-1">{{ form.errors.api_key }}</div>
                </div>
                <div>
                  <label class="block text-xs font-semibold text-on-surface mb-2">API Secret</label>
                  <input v-model="form.api_secret" type="password" placeholder="Enter your API secret" class="w-full bg-surface border border-outline-variant/15 rounded-lg px-4 py-2.5 text-sm font-mono text-on-surface placeholder:text-on-surface-variant/50 focus:border-primary focus:ring-1 focus:ring-pulse focus:outline-none transition-all" />
                ...
                <div v-if="selectedExchangeRequiresPassphrase">
                  <label class="block text-xs font-semibold text-on-surface mb-2">API Passphrase</label>
                  <input v-model="form.api_passphrase" type="password" placeholder="Enter your API passphrase" class="w-full bg-surface border border-outline-variant/15 rounded-lg px-4 py-2.5 text-sm font-mono text-on-surface placeholder:text-on-surface-variant/50 focus:border-primary focus:ring-1 focus:ring-pulse focus:outline-none transition-all" />
                </div>
              ...
              <template v-if="form.type === 'software'">
                <div>
                  <label class="block text-xs font-semibold text-on-surface mb-2">Wallet App</label>
                  <div class="grid grid-cols-3 gap-2">
                    <button
                ...
                <div>
                  ...
                  <select v-model="form.chain_id" class="w-full bg-surface border border-outline-variant/15 rounded-lg px-4 py-2.5 text-sm text-on-surface focus:border-primary focus:ring-1 focus:ring-pulse focus:outline-none transition-all cursor-pointer">
                ...
                <div>
                  <label class="block text-xs font-semibold text-on-surface mb-2">Wallet Address</label>
                  <input v-model="form.address" type="text" placeholder="0x... or bc1... or So1..." class="w-full bg-surface border border-outline-variant/15 rounded-lg px-4 py-2.5 text-sm font-mono text-on-surface placeholder:text-on-surface-variant/50 focus:border-primary focus:ring-1 focus:ring-pulse focus:outline-none transition-all" />
                  <div v-if="form.errors.address" class="text-red-500 text-xs mt-1">{{ form.errors.address }}</div>
                </div>
                <div>
                  <label class="block text-xs font-semibold text-on-surface mb-2">Label</label>
                  <input v-model="form.label" type="text" placeholder="e.g. My MetaMask" class="w-full bg-surface border border-outline-variant/15 rounded-lg px-4 py-2.5 text-sm text-on-surface placeholder:text-on-surface-variant/50 focus:border-primary focus:ring-1 focus:ring-pulse focus:outline-none transition-all" />
                  <div v-if="form.errors.label" class="text-red-500 text-xs mt-1">{{ form.errors.label }}</div>
                </div>
              </template>
              ...
              <template v-if="form.type === 'xpub'">
                <div>
                  <label class="block text-xs font-semibold text-on-surface mb-2">Label</label>
                  <input v-model="form.label" type="text" placeholder="e.g. Ledger Nano X" class="w-full bg-surface border border-outline-variant/15 rounded-lg px-4 py-2.5 text-sm text-on-surface placeholder:text-on-surface-variant/50 focus:border-primary focus:ring-1 focus:ring-pulse focus:outline-none transition-all" />
                  <div v-if="form.errors.label" class="text-red-500 text-xs mt-1">{{ form.errors.label }}</div>
                </div>
                <div>
                  <label class="block text-xs font-semibold text-on-surface mb-2">Blockchain</label>
                ...
                <div>
                  <label class="block text-xs font-semibold text-on-surface mb-2">Extended Public Key</label>
                  <textarea v-model="form.xpub_key" rows="3" placeholder="xpub6CUGRUonZSQ4TWtTMmzXdrXDtypWKi..." class="w-full bg-surface border border-outline-variant/15 rounded-lg px-4 py-2.5 font-mono text-xs text-on-surface placeholder:text-on-surface-variant/50 focus:border-primary focus:ring-1 focus:ring-pulse focus:outline-none transition-all resize-none"></textarea>
                ...
                <div>
                  ...
                  <input v-model="form.derivation_path" type="text" class="w-full bg-surface border border-outline-variant/15 rounded-lg px-4 py-2.5 text-sm font-mono text-on-surface placeholder:text-on-surface-variant/50 focus:border-primary focus:ring-1 focus:ring-pulse focus:outline-none transition-all" />
              ...
              <template v-if="form.type === 'manual'">
                <div>
                  <label class="block text-xs font-semibold text-on-surface mb-2">Label</label>
                  <input v-model="form.label" type="text" placeholder="e.g. Trading Wallet" class="w-full bg-surface border border-outline-variant/15 rounded-lg px-4 py-2.5 text-sm text-on-surface placeholder:text-on-surface-variant/50 focus:border-primary focus:ring-1 focus:ring-pulse focus:outline-none transition-all" />
                  <div v-if="form.errors.label" class="text-red-500 text-xs mt-1">{{ form.errors.label }}</div>
                </div>
                <div>
                  <label class="block text-xs font-semibold text-on-surface mb-2">Blockchain</label>
                ...
                <div>
                  <label class="block text-xs font-semibold text-on-surface mb-2">Wallet Address</label>
                  <input v-model="form.address" type="text" placeholder="Paste any blockchain address" class="w-full bg-surface border border-outline-variant/15 rounded-lg px-4 py-2.5 text-sm font-mono text-on-surface placeholder:text-on-surface-variant/50 focus:border-primary focus:ring-1 focus:ring-pulse focus:outline-none transition-all" />
                  <div v-if="form.errors.address" class="text-red-500 text-xs mt-1">{{ form.errors.address }}</div>
                </div>
            ...
            <div v-if="modalStep === 2" class="flex items-center justify-end gap-3 px-6 py-4 border-t border-outline-variant/15 bg-surface sticky bottom-0">
              <button
                @click="closeModal"
              ...
              <button
                ...
                class="px-5 py-2.5 rounded-lg bg-primary text-void text-sm font-bold hover:shadow-lg hover:shadow-pulse/20 hover:bg-[#E5C130] transition-all disabled:opacity-30 disabled:cursor-not-allowed flex items-center gap-2"
              >
