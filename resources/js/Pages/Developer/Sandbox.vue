<template>
  <DashboardLayout pageTitle="Sandbox" breadcrumb="SYSTEM > DEV > TESTNET" dashboardType="developer">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
      ...
      <div class="lg:col-span-2 border border-ledger-border bg-void rounded-xl p-6 shadow-sm">
        ...
        <div class="grid grid-cols-2 gap-5 mb-6">
          <div>
            ...
            <input v-model="simAmount" type="number" placeholder="100.00" class="w-full bg-ledger border border-ledger-border rounded-lg px-4 py-2.5 text-sm text-text-primary placeholder:text-text-muted/50 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none transition-all" />
          </div>
          <div>
            <label class="block text-xs font-semibold text-text-muted mb-1.5">Token</label>
            <select v-model="simToken" class="w-full bg-ledger border border-ledger-border rounded-lg px-4 py-2.5 text-sm text-text-primary focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none transition-all appearance-none cursor-pointer">
        ...
        <!-- Simulation Log -->
        <div v-if="simLog.length > 0" class="border border-ledger-border rounded-lg bg-ledger p-4 font-mono text-xs space-y-1.5 max-h-48 overflow-y-auto">
      ...
      <!-- Testnet Faucets -->
      <div class="space-y-6">
        <div class="border border-ledger-border bg-void rounded-xl p-6 shadow-sm">
          ...
          <div class="space-y-3">
            <a v-for="faucet in faucets" :key="faucet.chain" :href="faucet.url" target="_blank" class="flex items-center gap-3 p-3 border border-ledger-border rounded-xl hover:border-node/50 hover:bg-node/5 hover:shadow-lg transition-all group/f">
              <span class="w-8 h-8 rounded-full border border-ledger-border bg-ledger flex items-center justify-center text-sm font-bold group-hover/f:border-node group-hover/f:text-node transition-colors">{{ faucet.icon }}</span>
              <div>
                <div class="text-sm font-bold text-text-primary group-hover/f:text-node transition-colors">{{ faucet.chain }}</div>
                <div class="text-xs font-medium text-text-muted mt-0.5">Get test {{ faucet.symbol }}</div>
              </div>
              <svg class="w-4 h-4 text-text-muted ml-auto group-hover/f:text-node transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
            </a>
          </div>
        </div>
        ...
        <div class="border border-ledger-border bg-void rounded-xl p-6 shadow-sm">
          ...
          <div class="bg-ledger border border-ledger-border rounded-lg p-3 font-mono text-xs text-amber-500 break-all mb-3 text-center">
            tpub6CUGRonZSQ4TWtT...{{ generateHash(8) }}
          </div>
          <div class="text-xs font-medium text-text-muted text-center leading-relaxed">
    ...
    >
      ...
      <template #cell-success="{ row }">
        <span class="px-2 py-0.5 rounded-md text-[11px] font-semibold tracking-wide" :class="row.status_code >= 200 && row.status_code < 300 ? 'text-pulse bg-pulse/10' : 'text-red-500 bg-red-500/10'">{{ row.status_code }}</span>
      </template>
