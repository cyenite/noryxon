<template>
  <DashboardLayout pageTitle="Live Monitor" breadcrumb="SYSTEM > REALTIME > TERMINAL" dashboardType="merchant">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
      <div class="flex items-center gap-4">
        <div class="flex items-center gap-2">
          <span class="w-2.5 h-2.5 rounded-full bg-primary animate-pulse shadow-[0_0_8px] shadow-pulse"></span>
          <span class="text-sm font-semibold text-on-surface">Live Feed Active</span>
        </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <select 
          v-model="chainFilter"
          class="bg-surface border border-outline-variant/15 rounded-lg px-3 py-1.5 text-sm font-medium text-on-surface focus:border-primary focus:ring-1 focus:ring-pulse focus:outline-none transition-all cursor-pointer"
        >
        ...
        <button 
          ...
          :class="soundEnabled ? 'border-primary text-primary bg-primary/10' : 'border-outline-variant/15 text-on-surface-variant hover:border-primary hover:text-on-surface'"
        >
        ...
        <button 
          @click="events = []"
          class="px-3 py-1.5 rounded-lg border border-outline-variant/15 text-sm font-medium text-on-surface-variant hover:border-red-500 hover:text-red-500 transition-colors"
        >
          Clear
        </button>
      </div>
    ...
    <!-- Terminal Feed -->
    <div class="border border-outline-variant/15 rounded-xl bg-surface overflow-hidden shadow-sm">
      <!-- Terminal Header -->
      ...
      <div class="h-[65vh] overflow-y-auto p-4 space-y-0.5 font-mono text-xs" ref="feedContainer">
        <!-- Boot Messages -->
        <div class="text-on-surface-variant opacity-60 mb-4">
          <div>> NORYXON_INDEXER v2.1.0 — CONNECTING TO NODE CLUSTER...</div>
          <div>> WEBSOCKET_HANDSHAKE: <span class="text-primary">ESTABLISHED</span></div>
        ...
        >
          <div 
            ...
            :key="event.id"
            class="flex items-center gap-3 py-2 px-3 hover:bg-surface-container-low/50 transition-colors group/evt cursor-pointer rounded-md mx-1 my-0.5"
            :class="{
          ...
          >
            ...
            <span class="text-primary-container opacity-0 group-hover/evt:opacity-100 transition-opacity ml-auto text-[10px]">{{ event.tx_hash || event.address }}</span>
            <!-- Confirmations -->
            <div class="flex items-center gap-2 ml-auto shrink-0">
              <div class="w-12 h-1.5 bg-surface-container-lowest-border rounded-full overflow-hidden">
                <div 
